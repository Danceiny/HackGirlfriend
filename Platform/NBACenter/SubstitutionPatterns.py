...
save_path = concat_dirs(True, cur_file_path, '..', '..', '..', 'Applications', 'NBA', 'static')
...
def nba_substitution_crawler():
		...
		for comment in comments:
			...
			if team_stats:
				...
				for team_name in team_names:
					...
					with open(os.path.join(save_path,"data/" + year + "/" + team_abr + ".csv"), "wb") as f:
						writer = csv.writer(f) 
	...
	with open(os.path.join(save_path,"data/last_update.txt"), "wb") as last_update: last_update.write(("{d:%B} {"
																									   "d.day}, "
																					"{d.year}\n").format(d = today))
