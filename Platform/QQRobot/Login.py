...
class Login(HttpClient):
    def __init__(self, qq_number=0, params=None):
        ...
        self.VPath = VPATH % str(int(time.time()))  # QRCode保存路径
    ...
    def login(self):
        ...
        # 6. 扫码
        StarTime = util.date_to_millis(datetime.datetime.utcnow())
        ...
        while True:
            ...
            while True:
                login_html = self.Get((QQ_LOGIN_URL).format(util.getQRtoken(self.QRSig), self.APPID,
                                                            util.date_to_millis(datetime.datetime.utcnow()) - StarTime,
                                                            self.MiBaoCss, self.JS_VERSION, self.SIGN),UI_PTLOGIN2_URL)
                logging.info('[{0}] Get QQ_LOGIN_URL html, %s',login_html)
